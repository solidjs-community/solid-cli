...
export const replaceString = async (filePath: string, search: string | RegExp, replace: string) => {
  let contents = await readFileToString(filePath);
  contents = contents.replace(search, replace);
  await writeFile(filePath, contents);
};
export const insertBefore = async (filePath: string, search: string | RegExp, text: string) => {
  await replaceString(filePath, search, text + search);
};
export const insertAfter = async (filePath: string, search: string | RegExp, text: string) => {
